import { mock as data } from '../data/day6.js';

// We build an array with the amount of occurence
let sortedItemsByStartLife = [0];
for (var i = 0; i < data.length; i++) {
  sortedItemsByStartLife[data[i]] = (sortedItemsByStartLife[data[i]] || 0) + 1;
}
...
console.log(sortedItemsByStartLife);
...
function resolve() {
  ...
  for (var i = 0; i < sortedItemsByStartLife.length; i++) {
    if (sortedItemsByStartLife[i] > 0) {
      console.log(
        'start for ',
        i,
        'with a amount of lanterns of ',
        sortedItemsByStartLife[i]
      );

      var result = countAmountOfLanternWithAStartAt(i);

      sum += result * sortedItemsByStartLife[i];
    } else {
